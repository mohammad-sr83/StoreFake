...
const { resolvedTheme} = useTheme();
  ...
  return (
    <div
      dir={"rtl"}
      className="w-full h-1/2 flex justify-between items-center flex-col gap-3 px-6 pt-3 mb-3 bg-red-600 border-2 rounded-2xl"
    >
      <div className="w-full flex justify-between items-center content-start gap-3 py-3 rounded-2xl">
        <SwiperComponentButton>
          ...
          >
            <div className="w-full h-[22rem] sm:h-[24rem] flex justify-center gap-3 items-center flex-col text-white">
          ...
          {array.map((item, index) => (
            ...
            >
              <Link href={`store/${item.id}`}>
                <CardShop amazing="12" photo={item.photo} title={item.title} />
              </Link>
            </SwiperSlide>
          ))}
          <SwiperSlide
          ...
          >
            <div
              className={`w-full h-[22rem] sm:h-[24rem] flex justify-center items-center rounded-l-2xl flex-col gap-4 ${
                resolvedTheme=== "dark" ? "bg-black text-white" : "bg-white text-black"
              }`}
            >
              <span className="p-4 rounded-full border-2 text-Secendery">
                <ArrowLeft />
              </span>
